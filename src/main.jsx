      ...
      <svg width={size} height={size}>
        {slices.map((s,i)=>(
          <path key={i} d={s.path} fill={s.color} stroke="#fff" strokeWidth={2}>
            <title>{s.label}: {fP(s.pct*100)}</title>
          </path>
        ))}
      </svg>
...
function CompanyScreen({tok,uid,toast}){
  ...
  useEffect(()=>{
    getOne('company_info',tok,'user_id',uid).then(r=>{
      if(Array.isArray(r.data)&&r.data.length>0){setF(r.data[0]);setExistingId(r.data[0].id);}
      setLoading(false);
    });
  },[]);
  ...
  const save=async()=>{
    ...
    setSaving(false);
    if(res.ok){toast('Dados da empresa salvos!');const d=Array.isArray(res.data)?res.data[0]:res.data;if(d?.id)setExistingId(d.id);}
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div style={{background:'#fff',borderRadius:12,padding:24}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          ...
          <Inp label="Responsável / Sócio" val={f.owner||''} set={v=>s('owner',v)} type="text" ph="Rafael Quessa"/>
          <div style={{marginBottom:14}}>
        ...
        <Btn ch={saving?'Salvando...':'💾 Salvar Dados da Empresa'} onClick={save} v="gold" full disabled={saving}/>
      </div>
    </div>
  );
}
...
function gerarPDF(products,costs,fixedTotal,company){
  ...
  const addLine=(text,x,yy,opts={})=>lines.push({text,x,y:yy,opts});

  // Gera o PDF como HTML para impressão
  const semLabel={green:'SAUDÁVEL',yellow:'ATENÇÃO',red:'PREJUÍZO',none:'—'};
  const semColor={green:'#1A6B2A',yellow:'#B8860B',red:'#B0291E',none:'#999'};

  const rows=withCfu.map(p=>{
    const r=calc(p,p._cfu,fixedTotal);
    const pv=+p.current_price>0?+p.current_price:0;
    return{...p,r,pv};
  });
...
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  body{font-family:Arial,sans-serif;font-size:11px;color:#333;background:#fff;padding:20px;}
  ...
  .header h1{font-size:20px;color:#C9A84C;}
  .header p{font-size:11px;color:#B8C9E8;margin-top:4px;}
  .company-info{font-size:11px;color:#B8C9E8;text-align:right;}
  ...
  .card .value{font-size:16px;font-weight:900;color:#0F2744;}
  .card .sub{font-size:9px;color:#aaa;margin-top:2px;}
  .section-title{font-size:13px;font-weight:700;color:#0F2744;margin:16px 0 10px;padding-bottom:6px;border-bottom:2px solid #C9A84C;}
  ...
  .sem-green{background:#EAFAEE;color:#1A6B2A;}
  .sem-yellow{background:#FFF8E0;color:#B8860B;}
  .sem-red{background:#FFF0EE;color:#B0291E;}
  .sem-none{background:#f5f5f5;color:#999;}
  .pe-bar{background:#e8e8e8;border-radius:4px;height:8px;overflow:hidden;margin-top:4px;}
  .pe-fill{height:100%;border-radius:4px;}
...
<div class="header">
  <div>
    ...
    <p>Relatório Completo de Produtos · Método dos 3Cs · Rafael Quessa Contador</p>
    <p>Gerado em: ${new Date().toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})}</p>
  </div>
  <div class="company-info">
    ${company?.name?`<div style="font-size:14px;font-weight:700;color:#C9A84C;">${company.name}</div>`:''}
    ${company?.cnpj?`<div>CNPJ: ${company.cnpj}</div>`:''}
    ${company?.owner?`<div>${company.owner}</div>`:''}
    ${company?.regime?`<div>${company.regime}</div>`:''}
...
<div class="summary">
  <div class="card"><div class="label">Total de Produtos</div><div class="value">${products.length}</div><div class="sub">cadastrados</div></div>
  <div class="card"><div class="label">Custos Fixos/Mês</div><div class="value">${fR(fixedTotal)}</div><div class="sub">total mensal</div></div>
  <div class="card"><div class="label">Faturamento Estimado</div><div class="value">${fR(totalFat)}</div><div class="sub">preços atuais × volumes</div></div>
  <div class="card"><div class="label">PE Geral</div><div class="value">${fR(peGeral)}</div><div class="sub">faturamento mínimo</div></div>
</div>

<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;">
  <div class="card">
    <div class="label">Status dos Produtos</div>
    <div style="margin-top:8px;">
      ${['green','yellow','red'].map(s=>{
        const cnt=rows.filter(p=>p.r.sem===s).length;
        const pct=rows.length>0?(cnt/rows.length)*100:0;
        const colors={green:GRN,yellow:YLW,red:RED};
        const labels={green:'Saudável',yellow:'Atenção',red:'Prejuízo'};
        return`<div style="margin-bottom:6px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:2px;">
            <span style="font-size:10px;color:#555;">${labels[s]}</span>
            <span style="font-size:10px;font-weight:700;color:${colors[s]};">${cnt} (${fP(pct)})</span>
          </div>
          <div class="pe-bar"><div class="pe-fill" style="width:${pct}%;background:${colors[s]};"></div></div>
        </div>`;
      }).join('')}
    </div>
  </div>
  <div class="card">
    ...
    <div style="margin-top:8px;">
      <div style="font-size:18px;font-weight:900;color:#0F2744;">${fR(peGeral)}</div>
      <div style="font-size:10px;color:#aaa;margin-bottom:6px;">faturamento mínimo/mês</div>
      <div style="display:flex;justify-content:space-between;margin-bottom:2px;">
        <span style="font-size:10px;color:#555;">Progresso atual</span>
        <span style="font-size:10px;font-weight:700;color:${totalFat>=peGeral?GRN:RED};">${peGeral>0?fP(Math.min((totalFat/peGeral)*100,100)):'—'}</span>
      </div>
      <div class="pe-bar"><div class="pe-fill" style="width:${peGeral>0?Math.min((totalFat/peGeral)*100,100):0}%;background:${totalFat>=peGeral?GRN:RED};"></div></div>
      <div style="font-size:9px;color:#aaa;margin-top:4px;">MC% Ponderada: ${fP(mcPctPonderada)}</div>
    </div>
  </div>
</div>

<div class="section-title">📦 Relatório Detalhado de Produtos</div>
<table>
  <thead>
    <tr>
      <th>Produto</th><th>Tipo</th><th>Custo Direto</th><th>CF/Un.</th>
      <th>Preço Mínimo</th><th>Preço Ideal</th><th>Preço Atual</th>
      <th>MC%</th><th>Markup</th><th>PE (un.)</th><th>Status</th>
    </tr>
  </thead>
  <tbody>
    ${rows.map(p=>{
      const tl=PROD_TYPES.find(t=>t.k===p.type)?.l||'Produto';
      return`<tr>
        <td><strong>${p.name}</strong></td>
        <td>${tl}</td>
        <td>${fR(p.r.cdu)}</td>
        <td>${fR(p.r.cfu)}</td>
        <td style="color:#B0291E;font-weight:700;">${fR(p.r.pMin)}</td>
        <td style="color:#1A6B2A;font-weight:700;">${fR(p.r.pIdeal)}</td>
        <td>${p.pv>0?fR(p.pv):'—'}</td>
        <td>${fP(p.r.mcP)}</td>
        <td>${fN(p.r.mkp,2)}×</td>
        <td>${p.r.peUnits} un.</td>
        <td><span class="sem sem-${p.r.sem}">${semLabel[p.r.sem]}</span></td>
      </tr>`;
    }).join('')}
  </tbody>
</table>
...
<table>
  <thead><tr><th>Descrição</th><th>Valor Mensal</th><th>% do Total</th></tr></thead>
  <tbody>
    ${costs.map(c=>`<tr>
      <td>${c.name}</td>
      <td><strong>${fR(c.amount)}</strong></td>
      <td>${fixedTotal>0?fP((c.amount/fixedTotal)*100):'—'}</td>
    </tr>`).join('')}
    <tr style="background:#f0f4ff;">
      <td><strong>TOTAL</strong></td>
      <td><strong style="color:#0F2744;">${fR(fixedTotal)}</strong></td>
...
<div class="footer">
  <span>Precificação Inteligente · Método dos 3Cs · Rafael Quessa Contador</span>
  <span>rafaelquessacontador.com.br</span>
</div>
          ...
          {rows.map((i,idx)=>(
            <div key={i.id} style={{background:'#f8f9fc',borderRadius:10,padding:14,border:'1px solid #e0e0e0'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                <div><label style={{fontSize:11,color:'#888',display:'block',marginBottom:3}}>QTD USADA</label><input type="number" value={i.qty_used} onChange={e=>setIng(i.id,'qty_used',e.target.value)} placeholder="0" style={inp}/></div>
                <div><label style={{fontSize:11,color:'#888',display:'block',marginBottom:3}}>PREÇO PACOTE</label><input type="number" value={i.pkg_price} onChange={e=>setIng(i.id,'pkg_price',e.target.value)} placeholder="0,00" style={inp}/></div>
                <div><label style={{fontSize:11,color:'#888',display:'block',marginBottom:3}}>QTD PACOTE</label><input type="number" value={i.pkg_qty} onChange={e=>setIng(i.id,'pkg_qty',e.target.value)} placeholder="un." style={inp}/></div>
                <div><label style={{fontSize:11,color:'#888',display:'block',marginBottom:3}}>FRETE</label><input type="number" value={i.pkg_freight} onChange={e=>setIng(i.id,'pkg_freight',e.target.value)} placeholder="0,00" style={inp}/></div>
                <div><label style={{fontSize:11,color:'#888',display:'block',marginBottom:3}}>PERDA %</label><input type="number" value={i.loss_pct} onChange={e=>setIng(i.id,'loss_pct',e.target.value)} placeholder="0" style={inp}/></div>
              </div>
...
function InfoScreen(){
  const sections=[
    ...
    {e:'🏢',t:'Empresa',d:'Cadastre os dados da sua empresa: nome, CNPJ, regime tributário, contato. Essas informações aparecem no relatório PDF.'},
    {e:'🖨️',t:'Relatório PDF',d:'No Dashboard clique em "Gerar PDF" para exportar todos os produtos com preços, margens, semáforo e custos fixos.'},
...
function Dashboard({products,costs,fixedTotal,onOpenProd,tok,uid}){
  ...
  // Dados para gráficos
  const pieData=[
    {label:'Saudável',value:cnt.green,color:GRN},
    {label:'Atenção',value:cnt.yellow,color:YLW},
    {label:'Prejuízo',value:cnt.red,color:RED},
  ].filter(d=>d.value>0);

  const barMargem=withCfu.map(p=>{
    const r=calc(p,p._cfu,fixedTotal);
    return{label:p.name,value:Math.max(r.mcP,0),color:r.sem===GRN||r.mcP>=20?GRN:r.mcP>=10?YLW:RED,display:fP(r.mcP)};
  }).sort((a,b)=>b.value-a.value).slice(0,8);

  const barCustos=[
    {label:'Custos Diretos',value:withCfu.reduce((s,p)=>s+(+p.direct_cost||0)*Math.max(+p.monthly_volume||1,1),0),color:'#c0392b',display:''},
    {label:'Custos Fixos',value:fixedTotal,color:YLW,display:''},
    {label:'Lucro Estimado',value:Math.max(totalFat-withCfu.reduce((s,p)=>s+(+p.direct_cost||0)*Math.max(+p.monthly_volume||1,1),0)-fixedTotal,0),color:GRN,display:''},
  ];
  barCustos.forEach(b=>{b.display=fR(b.value);});
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      {products.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
          {/* Pizza — Status */}
          <div style={{background:'#fff',borderRadius:12,padding:20}}>
            <h4 style={{margin:'0 0 16px',color:NAVY,fontSize:14}}>🥧 Status dos Produtos</h4>
            {pieData.length>0
              ?<PieChart data={pieData} size={160}/>
              :<p style={{color:'#aaa',fontSize:13,textAlign:'center',padding:'20px 0'}}>Cadastre produtos com preço atual para ver o gráfico</p>
            }
          </div>
          {/* Pizza — Composição de custos */}
          <div style={{background:'#fff',borderRadius:12,padding:20}}>
            <h4 style={{margin:'0 0 16px',color:NAVY,fontSize:14}}>🥧 Composição do Faturamento</h4>
            {totalFat>0
              ?<PieChart data={barCustos.filter(b=>b.value>0).map(b=>({label:b.label,value:b.value,color:b.color}))} size={160}/>
              :<p style={{color:'#aaa',fontSize:13,textAlign:'center',padding:'20px 0'}}>Sem dados suficientes</p>
            }
          </div>
        </div>
      )}

      {/* Barras — Margem por produto */}
      {barMargem.length>0&&(
        <div style={{background:'#fff',borderRadius:12,padding:20}}>
          <h4 style={{margin:'0 0 16px',color:NAVY,fontSize:14}}>📊 Margem de Contribuição % por Produto</h4>
          <BarChart data={barMargem} maxVal={Math.max(...barMargem.map(d=>d.value),1)}/>
        </div>
      )}

      {/* Barras — Composição financeira */}
      {totalFat>0&&(
        <div style={{background:'#fff',borderRadius:12,padding:20}}>
          <h4 style={{margin:'0 0 16px',color:NAVY,fontSize:14}}>📊 Composição Financeira Mensal</h4>
          <BarChart data={barCustos} maxVal={Math.max(...barCustos.map(d=>d.value),1)}/>
        </div>
      )}
      ...
      {products.length>0&&(
        <div style={{background:'#fff',borderRadius:12,padding:24}}>
          ...
          <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:16}}>
            ...
            <Card title="Faturamento Estimado" value={fR(totalFat)} sub="Preços × volumes" color={totalFat>=peGeral?GRN:RED}/>
          ...
          <div style={{background:totalFat>=peGeral*1.2?'#eafaee':totalFat>=peGeral?'#fff8e0':'#fff0ee',border:`2px solid ${totalFat>=peGeral*1.2?GRN:totalFat>=peGeral?YLW:RED}`,borderRadius:10,padding:'14px 18px',marginTop:8}}>
            {totalFat>=peGeral*1.2
              ...
              :totalFat>=peGeral
              ?<p style={{margin:0,fontSize:14,fontWeight:700,color:YLW}}>⚠️ Acima do PE, mas abaixo da meta. Faltam {fR(peGeral*1.2-totalFat)}/mês.</p>
              :<p style={{margin:0,fontSize:14,fontWeight:700,color:RED}}>🔴 Abaixo do PE! Faltam {fR(peGeral-totalFat)}/mês para cobrir os custos.</p>
            }
          </div>
        </div>
      )}
...
function App(){
  ...
  const NAV=[['dash','📊','Início'],['products','📦','Produtos'],['costs','💰','Custos'],['company','🏢','Empresa'],['info','ℹ️','Sobre']];
